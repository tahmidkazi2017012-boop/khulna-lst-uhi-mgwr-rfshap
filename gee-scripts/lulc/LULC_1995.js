// =====================================================================
//  LULC classification (Random Forest) - Landsat 5 TM  |  Khulna, Bangladesh
//  Year: 1995
//
...
var annualComposite = landsatProcessed.median().unmask().clip(khulna);

// ---- Indices ----
// NDWI definition this year: Green-SWIR1 (modified NDWI)
var ndvi = annualComposite.normalizedDifference(["SR_B4", "SR_B3"]).rename("NDVI");
var ndwi = annualComposite.normalizedDifference(["SR_B2", "SR_B5"]).rename("NDWI");
var ndbi = annualComposite.normalizedDifference(["SR_B5", "SR_B4"]).rename("NDBI");
var builtIndex = ndbi.subtract(ndvi).rename("Built_Index");

var lulcFeatures = annualComposite
  .addBands(ndvi).addBands(ndwi).addBands(ndbi).addBands(builtIndex);
...
// ---- Post-processing (this year only): NDVI-based built-up correction,
//      gap fill, and focal-mode noise removal (faithful to original run) ----
var correctedBuilt = lulcMap.where(lulcMap.eq(1).and(ndvi.gt(0.3)), 3);
var finalMap = correctedBuilt.unmask(3).focal_mode(1);

// ---- Display ----
var lulcPalette = ['0000ff','ff0000','00ff00','ffff00','d2b48c'];
Map.addLayer(finalMap, {min:0, max:4, palette:lulcPalette}, "LULC 1995");
...
Export.image.toDrive({
  image: finalMap, description: '1995_LULC_Khulna',
